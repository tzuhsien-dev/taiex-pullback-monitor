...
const lowLabel = {
  rolling: 'N 日低點',
  zigzag: '已確認波段低點',
  volatilityAdjustedZigZag: '已確認波段低點',
};

const highLabel = {
  rolling: 'N 日高點',
  zigzag: '已確認波段高點',
  volatilityAdjustedZigZag: '已確認波段高點',
};
    ...
    <section className="rounded-lg border border-slate-800 bg-panel p-4 shadow-xl shadow-black/20">
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">顯示目前模式判定的高點、低點與回落門檻線</p>
        </div>
      ...
      <div className="h-[360px] w-full sm:h-[460px]">
        <ResponsiveContainer height="100%" width="100%">
          <LineChart data={data} margin={{ top: 18, right: 18, bottom: 8, left: 2 }}>
            ...
            <Line dataKey="rollingHigh" dot={false} name={highLabel[result.highLowMode]} stroke="#a78bfa" strokeDasharray="7 5" strokeWidth={2} type="monotone" />
            <Line dataKey="thresholdIndex" dot={false} name="回落門檻線" stroke="#34d399" strokeDasharray="5 5" strokeWidth={2} type="monotone" />
            <Line dataKey="rollingLow" dot={false} name={lowLabel[result.highLowMode]} stroke="#facc15" strokeDasharray="4 4" strokeWidth={2} type="monotone" />
            {highPoint ? (
              <ReferenceDot fill="#a78bfa" r={6} stroke="#ffffff" x={highPoint.date} y={highPoint.index}>
                <Label fill="#ddd6fe" position="top" value={highLabel[result.highLowMode]} />
              </ReferenceDot>
            ) : null}
            {lowPoint ? (
              <ReferenceDot fill="#facc15" r={6} stroke="#ffffff" x={lowPoint.date} y={lowPoint.index}>
                <Label fill="#fef3c7" position="bottom" value={lowLabel[result.highLowMode]} />
              </ReferenceDot>
            ) : null}
            {latestPoint ? (
