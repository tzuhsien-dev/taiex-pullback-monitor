...
可部署到 GitHub Pages 的純靜態台股指數監控工具。網站沒有後端、serverless function 或 `/api` route；使用者可在瀏覽器中手動向 TWSE 更新資料，近期資料與設定使用 `localStorage`，完整歷史資料使用 IndexedDB。
...
- `更新最新資料`：瀏覽器直接向 TWSE 讀取加權指數與加權報酬指數；已有完整歷史庫時更新 IndexedDB，否則儲存在 `localStorage`
- `下載完整歷史`：從官方最早月份分月下載，加權指數自 1999 年 1 月、加權報酬指數自 2003 年 1 月，儲存在 IndexedDB
- `清除本地資料`：清除瀏覽器中的手動更新與完整歷史資料，回到內建靜態 JSON 或範例資料
...
完整歷史下載會逐月寫入 IndexedDB。中途取消或失敗時，已完成月份會保留，下次按「補齊完整歷史」只下載缺少月份；目前月份會重新下載以補上最新交易日。完整下載可能耗時數分鐘。
