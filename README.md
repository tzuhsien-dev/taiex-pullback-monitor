...
- 加權指數：1999-01-05 至 2026-06-11，共 6,754 筆
- 加權報酬指數：2003-01-02 至 2026-06-11，共 5,703 筆

## 架構

- `public/data/*.json`：所有使用者共用的完整歷史基準資料
- 瀏覽器手動更新：只讀取最近兩個月份，與基準資料合併後存入 `localStorage`
- `scripts/update-data.mjs`：維護者在本機重新產生完整歷史資料
- `scripts/publish-data.mjs`：驗證資料、自動 commit 並推送三個資料檔
- 已開啟的頁面重新取得焦點時會檢查部署版本；若有新版，頁首會提示重新載入
...
只更新工作樹中的完整資料檔，不 commit 或 push：

```bash
npm run data:refresh
```

此指令會重新分月下載 TWSE 官方可取得的全部資料：
...
以下條件都成立時，可用單一指令完成下載、測試、build、commit 與 push：
...
發布腳本只允許上述三個資料檔產生變更，不使用 force push。任何下載、測試或 build 失敗都不會 commit 或推送；若指數資料沒有變化，會還原 metadata 並正常結束，不建立空 commit。

自動 commit 訊息格式：

```text
data: refresh TWSE history through YYYY-MM-DD
```
...
走勢圖預設顯示目前觀察期，並可透過下方日期選取器或「顯示全部」查看完整歷史。高點、低點與回落門檻虛線代表目前觀察期的計算值，會橫跨目前可視區；縮放不會改變計算結果，切換指數、觀察期或高低點模式後會回到最近觀察期。
